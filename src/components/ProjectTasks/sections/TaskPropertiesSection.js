    ...
    <div className="task-properties-section" style={{ marginBottom: '16px', padding: '12px', background: '#f8f9fa', borderRadius: '8px' }}>
      <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <label>任務類型：</label>
          <select value={selectedTask.taskType || 'recurring'} onChange={(e) => handleTaskDetailUpdate(selectedTask.id, 'taskType', e.target.value)}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div className="tag-dropdown-container" style={{ position: 'relative', minWidth: '150px' }}>
            <div
              ...
              style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 12px', border: '1px solid #E8EDF2', borderRadius: '4px', cursor: 'pointer', background: 'white' }}
            >
              <span>{selectedTask.tagId ? taskTags.find(t => t.id === selectedTask.tagId)?.name || '無屬性' : '無屬性'}</span>
            ...
            {tagDropdownOpen === 'detail' && (
              <div className="tag-dropdown" style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '1px solid #E8EDF2', borderRadius: '4px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', zIndex: 1000, maxHeight: '200px', overflowY: 'auto' }}>
